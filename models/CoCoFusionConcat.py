...
class CoCoFusionConcat(nn.Module):
    def __init__(self, embed_dim, num_heads, num_layers, backbone_dim, dropout_rate=0.4, n_classes=4):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, self.embed_dim * 3))
        ...
        transformer_configs["dim"] = self.embed_dim * 3

        self.final_transformer = Transformer(transformer_configs)
        self.final_norm = nn.LayerNorm(self.embed_dim * 3)

        self.mlp_head = nn.Sequential(
            nn.Linear(self.embed_dim * 3, self.n_classes),
        )
    ...
    def forward(self, x5_patch_features, x10_patch_features, x20_patch_features):
        ...
        x_fine_co, attn_weights = self.fine_transformer(x_fine_co, _H, _W)
        ...
        # Conv
        x = torch.cat((x10, x_fine_co, x_coarse_co), dim=-1)
